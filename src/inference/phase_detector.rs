...
impl PhaseDetector {
    ...
    pub fn advance_manually(&mut self) -> String {
    ...
    pub fn tick(&mut self, frame: &FrameBuffer) -> anyhow::Result<Option<PhaseChange>> {
    ...
    pub fn recognize_party_name(
        &self,
        frame: &FrameBuffer,
        crop: &CropArea,
    ) -> anyhow::Result<Option<String>> {
        let (model_w, model_h) = self.config.resolution.as_usize();
        let crop = crop.to_pixels(model_w, model_h);
        self.recognize_text_in_crop(frame, crop, self.config.ocr_upscale_factor)
    }
    ...
    fn recognize_text_in_crop(
        ...
        let (rgba_bytes, scaled_w, scaled_h) = preprocess_white_text_extraction(
            ...
            crop,
            scale_factor,
            self.config.white_text_threshold,
        );
